rbvj = function () {
  ...
  function addBall( _x, _y ) {
    ...
    var ball = {
      ...
      speed_x: random( -2, 2 ),
      speed_y: random( -5, -1 ),
      c: rgba( 255 ),//rgb((Math.random() * 255) + 1,(Math.random() * 255) + 1,(Math.random() * 255) + 1),
      sz: sz
    }
  ...
  draw = function () {
    ...
    //ctx.background( 255, 0.4 );//ctx.background( 0, 0.2 );
    ctx.background( 0, 0.2 );
    update();

    ctx.lineWidth = 4;
    ctx.strokeStyle = rgb( '#efcdb6' );
    for ( var i = 0; i < balls.length; i++ ) {
      b = balls[ i ];
      ctx.fillStyle = '#efcdb6';//b.c;
